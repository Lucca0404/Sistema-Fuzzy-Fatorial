...
* **Base de Regras Concisa (4 Regras):** Em conformidade com *Chang et al. (2025)*, o universo de discurso foi mapeado por apenas 4 termos linguísticos com antecedentes gaussianos. Esta restrição forte evitou o superajuste (*overfitting*) e a fragmentação de regras que ocorrem ao aplicar o limite genérico de 12 regras a conjuntos de dados reduzidos.
* **Regularização Ridge ($\lambda = 10^{-2}$):** De acordo com *Wiktorowicz (2020)*, a inclusão do fator de amortecimento estabilizou a inversão matricial dos mínimos quadrados no motor TSK, blindando o modelo contra matrizes singulares mal-condicionadas.
...
3. **Interpolação Fracionária Positiva:** Evidencia a precisão da reconstrução exponencial inversa. O modelo fuzzy conseguiu interpolar com precisão cirúrgica os pontos fracionários invisíveis no treino, colando-se à tendência da Função Gamma.

*(Dica: Guarde a imagem `resultado_experimento_positivo.png` gerada pelo teu Colab e faça o upload para o repositório. Depois, podes descomentar a linha abaixo para exibir o gráfico diretamente no README).*
---

## 📈 Consolidação Estatística Compartilhada (Protocolo de 5 Seeds)
...
| :--- | :---: | :---: |
| **Melhor MSE Encontrado (Espaço de Treino)** | *Inserir valor aqui* | *Inserir valor aqui* |
| **Desvio Padrão do MSE (Estabilidade)** | *Inserir valor aqui* | *Inserir valor aqui* |
| **Média do RMSE (Escala Combinatória Real)** | *Inserir valor aqui* | *Inserir valor aqui* |
| **Média do MAPE (%) (Escala Combinatória Real)**| *Inserir valor aqui*% | *Inserir valor aqui*% |
| **Tempo Médio de Execução (Segundos)** | *Inserir valor aqui* s | *Inserir valor aqui* s |
